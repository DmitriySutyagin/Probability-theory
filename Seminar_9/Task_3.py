...
alfa = 1e-6
for i in range(2000):
    B1 -= alfa * (2/n) * np.sum(((a+B1*x)-y)*x)/n
    if i % 100 == 0:
        print(f'Iterator = {i}, B1 = {B1}, mse = {mse_(B1, y, x, n)}')
print('Делаем вывод: модель рабочая т.к. значени B1 перестали изменятся на 1300-ой интерации. Также модель с интерсептом быстрее нашла решение')